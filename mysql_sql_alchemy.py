...
db_url = 'mysql+pymysql://username:password@localhost/mydb'
...
# Add table and record
@app.route('/add')
def insert():
    try:
        user_information = User(name='ismail', lname='salmi', email='ismail@gmail.com',
                                text='Im a python developer', password='1992')
...
def update():
    try:
        update_info = User.query.filter_by(id=7).first()
        update_info.id = 2
        update_info.name = 'admin'
        update_info.email = 'admin@gmail.com'
        db.session.commit()
